...
public class FlashlightToggle : MonoBehaviour 
{
	...
	void Start()
	{
		//HeadLight = GameObject.FindGameObjectWithTag( "Flashlight Light" );
...
//			Debug.Log(BatteryLife);
		...
		else if(Input.GetKeyDown(KeyCode.R))
		{
			if( BatteryLife >= 100 ) {
				...
				if (HeadLight.light.intensity >= 1 ) { HeadLight.light.intensity = 1; }
				else { HeadLight.light.intensity += ( BatteryReductionSpeed * Time.deltaTime ); }
			}
		}
